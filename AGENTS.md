...
- Streams the game via **YouTube Live API** — handles stream creation, management, and status
- Renders **real-time stat overlays** on top of the video feed (score, count, inning, batter info, pitch results)
- Overlays are driven by Supabase Realtime subscriptions to `game_state` and `lineup_entries` — same data Stats and Lineup write
- Supports **viewer reactions** (emoji bursts, comments) during the stream
- This app is the **heaviest consumer** of game state — it reads from Stats, Lineup, and potentially Sound (audio cues synced to stream)
- This app is the **authority** for: stream status, overlay layout/config, reactions, chat
- YouTube API has rate limits and quota — cache aggressively, batch API calls where possible
- Overlay rendering must be performant — updates happen every pitch, can't drop frames or lag behind the video
- Consider that viewers see the stream on a ~10-30 second delay from real time; overlay data arriving "too early" relative to the video is a UX problem
- Stream key and YouTube credentials must never be exposed client-side
